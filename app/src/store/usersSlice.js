...
export const createUsersSlice = (set, get) => ({
    ...
    resetUserProgress: (id) => {
        set((s) => {
            ...
            if (!user)
                return s;
            const fresh = { ...user, progress: emptyProgress() };
            const users = { ...s.users, [id]: fresh };
            persist({ activeUserId: s.activeUserId, users });
            return { users };
        });
    },
    recordAnswer: ({ userId, questionId, topicId, totalQuestionsInTopic, correct, attemptIndex, }) => {
        set((s) => {
            ...
            const attempts = Math.min(3, (prev?.attempts ?? 0) + 1);
            ...
            const updatedQ = {
                ...
                lastSeen: Date.now(),
                inReviewQueue: !becameMastered && attempts === 3 ? true : (prev?.inReviewQueue ?? false),
            };
            ...
            const mastered = (topicPrev?.mastered ?? 0) + (!wasMastered && updatedQ.mastered ? 1 : 0);
            const updatedT = {
                attempted,
                mastered,
    ...
    enqueueReview: (userId, questionId) => {
        set((s) => {
            const user = s.users[userId];
            if (!user || user.progress.reviewQueue.includes(questionId))
                return s;
            const updatedUser = {
                ...user,
                progress: {
